...
public class DataInitializer implements CommandLineRunner {
	...
	public void run(String... args) {
		...
		if (!evidenceRepository.findByUploaderIdAndStatusAndDeletedAtIsNullOrderByUploadedAtDesc(
				demoUser.getUserId(), EvidenceStatus.UPLOADED).isEmpty()) {
			return;
		}

		LocalDateTime now = LocalDateTime.now();
		seedCase(demoUser, "가세연 녹취록 딥페이크 의혹 사건", AnalysisStatus.ANALYZING, now.minusDays(1), 2);
		seedCase(demoUser, "CCTV 영상 위변조 검증 요청", AnalysisStatus.COMPLETED, now.minusDays(4), 5);
		seedCase(demoUser, "음성 메일 증거 분석", null, now.minusDays(2), 1);
		seedCase(demoUser, "인터뷰 클립 진위 확인", AnalysisStatus.FAILED, now.minusDays(9), 3);
	}

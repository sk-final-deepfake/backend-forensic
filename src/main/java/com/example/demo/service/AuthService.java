...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        User user = userRepository.findByLoginIdAndDeletedAtIsNull(request.getLoginId())
                .orElseThrow(() -> new AuthException(HttpStatus.UNAUTHORIZED, "사번 또는 비밀번호가 올바르지 않습니다."));

        if (!passwordEncoder.matches(request.getPassword(), user.getPassword())) {
            throw new AuthException(HttpStatus.UNAUTHORIZED, "사번 또는 비밀번호가 올바르지 않습니다.");
        }
    ...
    private void validateApprovedStatus(User user) {
        ...
        throw new AuthException(HttpStatus.FORBIDDEN, message);
    }
}

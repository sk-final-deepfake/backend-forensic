...
### 4.4 RabbitMQ 큐 등록 결과 및 실패 로그

추가 파일:

```text
src/main/java/com/example/demo/messaging/AnalysisQueueMessage.java
src/main/java/com/example/demo/messaging/AnalysisQueuePublisher.java
src/main/java/com/example/demo/messaging/RabbitMqAnalysisQueuePublisher.java
```
...
RabbitMQ publish 성공 payload에 queueRegistered=true 저장
RabbitMQ publish 실패 시 ERROR_OCCURRED 저장
RabbitMQ publish 실패 시 AnalysisRequest.status=FAILED 저장
비ERD 이벤트명 미사용
```

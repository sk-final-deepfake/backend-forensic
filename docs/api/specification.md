...
`createCase()`는 `POST /api/v1/cases`로 서버에 `case_profiles`를 먼저 생성하고, 이후 `POST /evidences/upload?caseName=`으로 증거를 추가합니다.
...
- **Errors:** `DUPLICATE_CASE_NAME` (409), `INVALID_REQUEST` (400)
